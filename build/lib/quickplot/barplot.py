...
def barplot(df, xvar, yvar, title=None, xlab=None, ylab=None, 
            y_as_pct=False, y_as_usd=False, y_decimals=0, yticks=8,
            color_above_zero=fivethirtyeight_green, 
            color_below_zero=fivethirtyeight_red, 
            alpha=0.8, width=10, height=4, bar_label_decimals=0, 
            ):
    """
    Plot vertical bar charts where bars below 0 are red and above are green.
    ...
    yticks : int  (default = 8)
        Number of y-ticks. 
    color_above_zero : str (default = '#77AB43')
        Color for bars > 0.
    color_below_zero : str (default = '#FF2700')
        Color for bars < 0.
    ...
    g = sns.catplot(
        data=df, x=xvar, y=yvar, kind='bar', alpha=alpha,
        palette=(df[yvar] > 0).map({True:color_above_zero, False:color_below_zero}),
        legend=False)
    if xlab is None: 
        xlab = xvar    
    if ylab is None:
