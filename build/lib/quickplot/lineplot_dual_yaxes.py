...
def lineplot_double_yaxes(
    ...
    left_y_decimals=0, right_y_decimals=0, left_yticks=None, right_yticks=None,
    alpha=0.8, width=10, height=4):
    """
    ...
    # left y
    ax1 = df[left_yvar].plot(color=fivethirtyeight_blue, alpha=alpha)
    ...
    # right y
    ax2 = ax1.twinx()
    ax2.plot(df.index, df[right_yvar], color=fivethirtyeight_green, alpha=alpha)
    ...
    if left_yticks is not None:
        left_y = df[left_yvar]
        steps = (left_y.max() - left_y.min()) / left_yticks
        ax1.set_yticks(np.arange(left_y.min(), left_y.max()+steps, steps))
    if right_yticks is not None:
        right_y = df[right_yvar]
        steps = (right_y.max() - right_y.min()) / right_yticks
        ax2.set_yticks(np.arange(right_y.min(), right_y.max()+steps, steps))
